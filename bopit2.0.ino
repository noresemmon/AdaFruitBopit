...
volatile bool level1Flag,level2Flag,level3Flag,level4Flag,level5Flag,level6Flag,resetFlag,level7Flag = false;
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(interruptSwitch), Switch, CHANGE);
   attachInterrupt(digitalPinToInterrupt(interruptLeftButton), leftButton, FALLING);
  attachInterrupt(digitalPinToInterrupt(interruptRightButton), rightButton, FALLING);
Serial.begin(9600);
}
...
level();
 ...
 if (gameOver){
    ...
    Serial.println("Type yes into the serial monitor!");
    resetFlag = true;
    }
  }
}
